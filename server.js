...
const SHEET_ID = "15Xg4nlQIK6FCFrCAli8qgKvWtwtDzXjBmVFHwYgF2TI"; // Tu hoja de cálculo
...
const BADGE_INFO = {
    1: "Certificación Especialista SST - Nivel I",
...
// Configuración de la Wallet Admin
if (PRIVATE_KEY) {
    wallet = new ethers.Wallet(PRIVATE_KEY, provider);
    contract = new ethers.Contract(CONTRACT_ADDRESS, CONTRACT_ABI, wallet);
    console.log(`🤖 Bot SST DAO Iniciado. Wallet Admin: ${wallet.address}`);
} else {
    console.log("⚠️ ADVERTENCIA: No has configurado la ADMIN_PRIVATE_KEY en las variables de entorno.");
}

// === FUNCIÓN INTELIGENTE: Leer Google Sheets en Vivo ===
...
app.get('/', (req, res) => {
    res.send("✅ Servidor SST DAO Activo y escuchando.");
});
...
app.post('/api/emitir-insignia', async (req, res) => {
    try {
        ...
        console.log(`🔎 Buscando en Google Sheets: ${email}...`);

        // 1. BUSCAR EN GOOGLE SHEETS EN TIEMPO REAL
        const walletDestino = await buscarWalletEnSheet(email);

        if (!walletDestino) {
            return res.status(404).json({ error: "No encontramos este correo en tu registro. Asegúrate de haber llenado el formulario de Google." });
        }

        console.log(`✅ Encontrado. Enviando insignia ${badgeId} a ${walletDestino}`);

        // 2. EMITIR EN BLOCKCHAIN
        const tx = await contract.mintInsignia(walletDestino, badgeId, 1);
        console.log(`🚀 Transacción enviada: ${tx.hash}`);
        
        // Esperamos confirmación (aprox 2-5 segundos en Polygon)
        await tx.wait();

        // 3. GENERAR ENLACES DE ÉXITO
        const openSeaUrl = `https://opensea.io/assets/matic/${CONTRACT_ADDRESS}/${badgeId}`;
        const nombreCurso = BADGE_INFO[badgeId] || "Certificación SST DAO";
        ...
        res.status(500).json({ error: "Ocurrió un error al procesar la insignia. Intenta de nuevo en unos segundos." });
    }
});
